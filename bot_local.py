...
def send_welcome(message):
    ...
    chat_id = message.from_user.id  #___

    first_name = message.from_user.first_name   #___
    last_name = message.from_user.last_name     #___
    ...
    with open('template_text/welcome.txt', mode='r', encoding='utf-8') as f:
        ...
        welcome = temp.substitute(FULL_NAME = first_name+' '+last_name)
...
def send_about(message):
    ...
    with open('template_text/about.txt', mode='r', encoding='utf-8') as f:
        ...
        about = temp.substitute(
            STUDENT_NAME = 'Rakhmat Suryahadi',  #___ = ___,
            BATCH_ACADEMY = 'ZEUS',  #___ = ___,
            GITHUB_REPO_LINK = 'https://github.com/rakhmatsuryahadi/capstone'   #___ = ___,
        )
...
df['campaign_id'] = df['campaign_id'].astype('object')   #___
unique_campaign = df['campaign_id'].astype('string').unique()   #___
...
def send_summary(message):
    ...
    if selected_campaign_id in unique_campaign:
        ...
        start_date = df_campaign['reporting_date'].min().strftime(format='%d %b %Y')   #___
        end_date = df_campaign['reporting_date'].max().strftime(format='%d %b %Y')  #___
        
        # TO DO: perform calculation
        total_spent = df_campaign.spent.sum()   #___
        total_conversion = df_campaign.total_conversion.sum()   #___
        cpc = total_spent / total_conversion    #___
        ...
        with open('template_text/summary.txt', mode='r', encoding='utf-8') as f:
            ...
            summary = temp.substitute(
                CAMPAIGN_ID = selected_campaign_id, #___,
                START_DATE = start_date,    #___,
                END_DATE = end_date,    #___,
                TOTAL_SPENT = f"${total_spent:,.0f}",  #___,
                TOTAL_CONVERSION = f"${total_conversion:,.0f}", #___,
                CPC = f"${cpc:,.1f}"    #___
            )
...
#         plt.xlabel(___)
        ...
        plot_info = list(zip(['total spent', 'total approved conversion', 'total cpc'],
        df_plot.idxmax(),
        df_plot.idxmin()))
